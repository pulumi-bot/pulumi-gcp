...
type budgetArgs struct {
	// Defines notifications that are sent on every update to the billing account's spend, regardless of the thresholds defined
	// using threshold rules.
	AllUpdatesRule *BudgetAllUpdatesRuleArgs `pulumi:"allUpdatesRule"`
	// The budgeted amount for each usage period.
	Amount BudgetAmountArgs `pulumi:"amount"`
	// ID of the billing account to set a budget on.
	BillingAccount string `pulumi:"billingAccount"`
	// Filters that define which resources are used to compute the actual spend against the budget.
	BudgetFilter *BudgetBudgetFilterArgs `pulumi:"budgetFilter"`
	// User data for display name in UI. Must be <= 60 chars.
	DisplayName *string `pulumi:"displayName"`
	// Rules that trigger alerts (notifications of thresholds being crossed) when spend exceeds the specified percentages of
	// the budget.
	ThresholdRules []BudgetThresholdRuleArgs `pulumi:"thresholdRules"`
}
...
type BudgetArgs struct {
	// Defines notifications that are sent on every update to the billing account's spend, regardless of the thresholds defined
	// using threshold rules.
	AllUpdatesRule BudgetAllUpdatesRuleArgsPtrInput
	// The budgeted amount for each usage period.
	Amount BudgetAmountArgsInput
	// ID of the billing account to set a budget on.
	BillingAccount pulumi.StringInput
	// Filters that define which resources are used to compute the actual spend against the budget.
	BudgetFilter BudgetBudgetFilterArgsPtrInput
	// User data for display name in UI. Must be <= 60 chars.
	DisplayName pulumi.StringPtrInput
	// Rules that trigger alerts (notifications of thresholds being crossed) when spend exceeds the specified percentages of
	// the budget.
	ThresholdRules BudgetThresholdRuleArgsArrayInput
}
