 ...
 * import * as pulumi from "@pulumi/pulumi";
 * import * as gcp from "@pulumi/gcp";
 * import * as null from "@pulumi/null";
 * 
 * const null_data_source_auth_netw_postgres_allowed_2: Output<null.DATASOURCEResult>[] = [];
 * for (let i = 0; i < 2; i++) {
 *     null_data_source_auth_netw_postgres_allowed_2.push(pulumi.output(null.DataSource({
 *         inputs: [{
 *             name: `onprem-${(i + 1)}`,
 *             value: ["192.168.1.2", "192.168.2.3"][i],
 *         }],
 *     })));
 * }
 * const google_compute_instance_apps: gcp.compute.Instance[] = [];
 * for (let i = 0; i < 8; i++) {
 *     google_compute_instance_apps.push(new gcp.compute.Instance(`apps-${i}`, {
 *         bootDisk: {
 *             initializeParams: {
 ...
 *     }));
 * }
 * const null_data_source_auth_netw_postgres_allowed_1: Output<null.DATASOURCEResult>[] = [];
 * for (let i = 0; i < pulumi.all(google_compute_instance_apps.map(v => v.selfLink)).apply(__arg0 => __arg0.map(v => v).length); i++) {
 *     null_data_source_auth_netw_postgres_allowed_1.push(pulumi.output(null.DataSource({
 *         inputs: [{
 *             name: `apps-${(i + 1)}`,
 *             value: pulumi.all(google_compute_instance_apps.map(v => v.networkInterfaces)).apply(__arg0 => __arg0.map(v => v[0].accessConfig.0.natIp)[i]),
 *         }],
 *     })));
 * }
 * const google_sql_database_instance_postgres = new gcp.sql.DatabaseInstance("postgres", {
 *     databaseVersion: "POSTGRES_9_6",
 *     name: "postgres-instance",
 *     settings: {
 *         ipConfiguration: {
 *             authorizedNetworks: [
 *                 pulumi.all(null_data_source_auth_netw_postgres_allowed_1).apply(__arg0 => __arg0.map(v => v.outputs)),
 *                 pulumi.all(null_data_source_auth_netw_postgres_allowed_2).apply(__arg0 => __arg0.map(v => v.outputs)),
 *             ],
 *         },
 *         tier: "db-f1-micro",
 *     },
