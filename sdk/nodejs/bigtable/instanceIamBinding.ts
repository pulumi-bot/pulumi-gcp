 ...
 * 
 * * `google.bigtable.InstanceIamPolicy`: Authoritative. Sets the IAM policy for the instance and replaces any existing policy already attached.
 * * `google.bigtable.InstanceIamBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the instance are preserved.
 * * `google.bigtable.InstanceIamMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the instance are preserved.
 * 
 * > **Note:** `google.bigtable.InstanceIamPolicy` **cannot** be used in conjunction with `google.bigtable.InstanceIamBinding` and `google.bigtable.InstanceIamMember` or they will fight over what your policy should be. In addition, be careful not to accidentaly unset ownership of the instance as `google.bigtable.InstanceIamPolicy` replaces the entire policy.
 * 
 * > **Note:** `google.bigtable.InstanceIamBinding` resources **can be** used in conjunction with `google.bigtable.InstanceIamMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class InstanceIamBinding extends pulumi.CustomResource {
     ...
     * `google.bigtable.InstanceIamBinding` can be used per role. Note that custom roles must be of the format
