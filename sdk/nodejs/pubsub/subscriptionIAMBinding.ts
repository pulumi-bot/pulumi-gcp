 ...
 * 
 * * `google.pubsub.SubscriptionIAMPolicy`: Authoritative. Sets the IAM policy for the subscription and replaces any existing policy already attached.
 * * `google.pubsub.SubscriptionIAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the subscription are preserved.
 * * `google.pubsub.SubscriptionIAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the subscription are preserved.
 * 
 * > **Note:** `google.pubsub.SubscriptionIAMPolicy` **cannot** be used in conjunction with `google.pubsub.SubscriptionIAMBinding` and `google.pubsub.SubscriptionIAMMember` or they will fight over what your policy should be.
 * 
 * > **Note:** `google.pubsub.SubscriptionIAMBinding` resources **can be** used in conjunction with `google.pubsub.SubscriptionIAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class SubscriptionIAMBinding extends pulumi.CustomResource {
     ...
     * `google.pubsub.SubscriptionIAMBinding` can be used per role. Note that custom roles must be of the format
