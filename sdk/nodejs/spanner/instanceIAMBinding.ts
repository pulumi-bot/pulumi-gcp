 ...
 * 
 * * `google.spanner.InstanceIAMPolicy`: Authoritative. Sets the IAM policy for the instance and replaces any existing policy already attached.
 * 
 * > **Warning:** It's entirely possibly to lock yourself out of your instance using `google.spanner.InstanceIAMPolicy`. Any permissions granted by default will be removed unless you include them in your config.
 * 
 * * `google.spanner.InstanceIAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the instance are preserved.
 * * `google.spanner.InstanceIAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the instance are preserved.
 * 
 * > **Note:** `google.spanner.InstanceIAMPolicy` **cannot** be used in conjunction with `google.spanner.InstanceIAMBinding` and `google.spanner.InstanceIAMMember` or they will fight over what your policy should be.
 * 
 * > **Note:** `google.spanner.InstanceIAMBinding` resources **can be** used in conjunction with `google.spanner.InstanceIAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class InstanceIAMBinding extends pulumi.CustomResource {
     ...
     * `google.spanner.InstanceIAMBinding` can be used per role. Note that custom roles must be of the format
