 ...
 * 
 * * `google.spanner.DatabaseIAMPolicy`: Authoritative. Sets the IAM policy for the database and replaces any existing policy already attached.
 * 
 * > **Warning:** It's entirely possibly to lock yourself out of your database using `google.spanner.DatabaseIAMPolicy`. Any permissions granted by default will be removed unless you include them in your config.
 * 
 * * `google.spanner.DatabaseIAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the database are preserved.
 * * `google.spanner.DatabaseIAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the database are preserved.
 * 
 * > **Note:** `google.spanner.DatabaseIAMPolicy` **cannot** be used in conjunction with `google.spanner.DatabaseIAMBinding` and `google.spanner.DatabaseIAMMember` or they will fight over what your policy should be.
 * 
 * > **Note:** `google.spanner.DatabaseIAMBinding` resources **can be** used in conjunction with `google.spanner.DatabaseIAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class DatabaseIAMMember extends pulumi.CustomResource {
     ...
     * `google.spanner.DatabaseIAMBinding` can be used per role. Note that custom roles must be of the format
