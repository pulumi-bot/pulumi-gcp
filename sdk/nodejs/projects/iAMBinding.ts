 ...
 * 
 * * `google.projects.IAMPolicy`: Authoritative. Sets the IAM policy for the project and replaces any existing policy already attached.
 * * `google.projects.IAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the project are preserved.
 * * `google.projects.IAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the project are preserved.
 * 
 * > **Note:** `google.projects.IAMPolicy` **cannot** be used in conjunction with `google.projects.IAMBinding` and `google.projects.IAMMember` or they will fight over what your policy should be.
 * 
 * > **Note:** `google.projects.IAMBinding` resources **can be** used in conjunction with `google.projects.IAMMember` resources **only if** they do not grant privilege to the same role.
 * 
 * ## google\_project\_iam\_policy
 * 
 * > **Be careful!** You can accidentally lock yourself out of your project
 *    using this resource. Deleting a `google.projects.IAMPolicy` removes access
 *    from anyone without organization-level access to the project. Proceed with caution.
 *    It's not recommended to use `google.projects.IAMPolicy` with your provider project
...
export class IAMBinding extends pulumi.CustomResource {
    ...
    /**
     * The project ID. If not specified for `google.projects.IAMBinding`
     * or `google.projects.IAMMember`, uses the ID of the project configured with the provider.
     * Required for `google.projects.IAMPolicy` - you must explicitly set the project, and it
     * will not be inferred from the provider.
     */
    public readonly project!: pulumi.Output<string>;
    /**
     * The role that should be applied. Only one
     * `google.projects.IAMBinding` can be used per role. Note that custom roles must be of the format
