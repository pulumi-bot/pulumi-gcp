 ...
 * 
 * * `google.kms.KeyRingIAMPolicy`: Authoritative. Sets the IAM policy for the key ring and replaces any existing policy already attached.
 * * `google.kms.KeyRingIAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the key ring are preserved.
 * * `google.kms.KeyRingIAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the key ring are preserved.
 * 
 * > **Note:** `google.kms.KeyRingIAMPolicy` **cannot** be used in conjunction with `google.kms.KeyRingIAMBinding` and `google.kms.KeyRingIAMMember` or they will fight over what your policy should be.
 * 
 * > **Note:** `google.kms.KeyRingIAMBinding` resources **can be** used in conjunction with `google.kms.KeyRingIAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class KeyRingIAMMember extends pulumi.CustomResource {
     ...
     * `google.kms.KeyRingIAMBinding` can be used per role. Note that custom roles must be of the format
