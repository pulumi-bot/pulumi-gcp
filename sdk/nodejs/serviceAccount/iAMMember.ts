 ...
 * 
 * * `google.serviceAccount.IAMPolicy`: Authoritative. Sets the IAM policy for the service account and replaces any existing policy already attached.
 * * `google.serviceAccount.IAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the service account are preserved.
 * * `google.serviceAccount.IAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the service account are preserved.
 * 
 * > **Note:** `google.serviceAccount.IAMPolicy` **cannot** be used in conjunction with `google.serviceAccount.IAMBinding` and `google.serviceAccount.IAMMember` or they will fight over what your policy should be.
 * 
 * > **Note:** `google.serviceAccount.IAMBinding` resources **can be** used in conjunction with `google.serviceAccount.IAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class IAMMember extends pulumi.CustomResource {
     ...
     * `google.serviceAccount.IAMBinding` can be used per role. Note that custom roles must be of the format
