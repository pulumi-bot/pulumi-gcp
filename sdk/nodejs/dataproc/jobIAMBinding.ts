 ...
 * 
 * * `google.dataproc.JobIAMPolicy`: Authoritative. Sets the IAM policy for the job and replaces any existing policy already attached.
 * * `google.dataproc.JobIAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the job are preserved.
 * * `google.dataproc.JobIAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the job are preserved.
 * 
 * > **Note:** `google.dataproc.JobIAMPolicy` **cannot** be used in conjunction with `google.dataproc.JobIAMBinding` and `google.dataproc.JobIAMMember` or they will fight over what your policy should be. In addition, be careful not to accidentaly unset ownership of the job as `google.dataproc.JobIAMPolicy` replaces the entire policy.
 * 
 * > **Note:** `google.dataproc.JobIAMBinding` resources **can be** used in conjunction with `google.dataproc.JobIAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class JobIAMBinding extends pulumi.CustomResource {
     ...
     * `google.dataproc.JobIAMBinding` can be used per role. Note that custom roles must be of the format
