 ...
 * 
 * * `google.dataproc.ClusterIAMPolicy`: Authoritative. Sets the IAM policy for the cluster and replaces any existing policy already attached.
 * * `google.dataproc.ClusterIAMBinding`: Authoritative for a given role. Updates the IAM policy to grant a role to a list of members. Other roles within the IAM policy for the cluster are preserved.
 * * `google.dataproc.ClusterIAMMember`: Non-authoritative. Updates the IAM policy to grant a role to a new member. Other members for the role for the cluster are preserved.
 * 
 * > **Note:** `google.dataproc.ClusterIAMPolicy` **cannot** be used in conjunction with `google.dataproc.ClusterIAMBinding` and `google.dataproc.ClusterIAMMember` or they will fight over what your policy should be. In addition, be careful not to accidentaly unset ownership of the cluster as `google.dataproc.ClusterIAMPolicy` replaces the entire policy.
 * 
 * > **Note:** `google.dataproc.ClusterIAMBinding` resources **can be** used in conjunction with `google.dataproc.ClusterIAMMember` resources **only if** they do not grant privilege to the same role.
 * 
...
export class ClusterIAMBinding extends pulumi.CustomResource {
     ...
     * `google.dataproc.ClusterIAMBinding` can be used per role. Note that custom roles must be of the format
